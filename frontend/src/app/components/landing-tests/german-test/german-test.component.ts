...
export class GermanTestComponent implements AfterViewInit, OnInit {
  ...
  processAnswer(event: { field: string; answer: string }) {
    ...
    if (this.questionIndex > this.formLength) {
      ...
      const task = germanQuiz.find((q) => q.index == this.questionIndex);
